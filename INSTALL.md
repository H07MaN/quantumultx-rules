...
- 1,529 条原生重写规则，包括 reject 系列、两条选定的广告响应字段替换和 61 条重定向。
- 8,417 条域名拦截规则；删除了命中本重写服务主机的部分域名拦截冲突。
- 1,085 个去重的 MitM 主机名/模式，取自保留规则所属分组与上游已启用主机名的交集。分组中可能仍包含只供其他规则使用的主机名，并非逐个实测清单。
- 选取了 508 个上游分组中的规则。分组数不等于支持或实测通过的 App 数。
...
同步 fmz200 2026-09-10 上游重写内容。构建器现在按规范化后的 URL 正则去重，移除一条仅转义写法不同的百度重复规则；明确拒绝跨顶级域的 MitM 通配声明，并排除整个 `tiktokv.com` 服务域名的 MitM，保留具体广告请求路径。新增 MitM 主机重新参与分流冲突筛除。GeQ1an 分流和重写补充源本次哈希未变化；61 条重定向、6 处循环保护、两条响应替换及两个实测横幅域名补丁保持不变。
